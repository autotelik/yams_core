  ...
  context 'POST' do
    context 'upload e' do
      ...
      it 'when all params valid - stores a wav file as a new track' do
        parameters = { track: attributes_for(:track, :with_audio_upload_fixture) }

        expect { post '/tracks', params: parameters }.to change(YamsCore::Track, :count).by(1)

        expect(YamsCore::Track.last.audio.filename).to eq "test.wav"
      ...
      it 'sets suitable error notice when no title provided' do
        parameters = { track: attributes_for(:track, :with_audio_upload_fixture).except(:title) }
