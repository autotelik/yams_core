...
describe YamsCore::UserPolicy do
  subject { YamsCore::UserPolicy }
  ...
  permissions :index? do
    it 'prevents access if not an admin' do
      expect(YamsCore::UserPolicy).not_to permit(current_user)
    end
    it 'allows access for an admin' do
      expect(YamsCore::UserPolicy).to permit(admin)
    end
  end
