...
describe YamsCore::Available do
  ...
  context 'Downstream' do
    ...
    it 'publishes an event when an object such as a track is made available' do
      create_track
      expect(event_store).to have_published(an_event(YamsEvents::ItemMadeAvailableEvent))
    end
    ...
    it 'uses the available meta data to set price and currency on new Product', ffs: true do
